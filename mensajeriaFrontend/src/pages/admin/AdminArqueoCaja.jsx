...
function getUserDataFromToken() {
  ...
  if (!validarReporte()) {
    setError('Por favor corrija los errores en el formulario de reporte');
    return;
  }
  ...
  try {
    ...
    if (!userData.tenantId || !userData.empresaId) {
      setError('Error: No se pudieron obtener los datos de autenticación. Por favor, inicie sesión nuevamente.');
      return;
    }
    
    const url = `/proxy/api/reportes/arqueos?tenantId=${userData.tenantId}&mensajeriaId=${userData.empresaId}`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        fechaDesde: reporteForm.fechaDesde,
        fechaHasta: reporteForm.fechaHasta,
        formato: reporteForm.formato,
        incluirDetalles: reporteForm.incluirDetalles
      })
    });
    ...
    const extension = reporteForm.formato === 'pdf' ? 'pdf' : 'xlsx';
    const nombreArchivo = `reporte_arqueos_${reporteForm.fechaDesde}_${reporteForm.fechaHasta}_${fechaActual}.${extension}`;
...
const esTurnoEnHorario = (turno, horaActual) => {
  ...
  const obtenerTurnosDisponibles = async (fecha = new Date(), arqueoEnEdicion = null) => {
    try {
      const tiposTurnoResponse = await fetch(`/proxy/api/tipos-turno`, {
      ...
      for (const turno of tiposTurno) {
        try {
          const arqueoResponse = await fetch(
            `/proxy/api/arqueo-caja/actual?turnoId=${turno.id}&fecha=${fechaString}`,
            {
              headers: getAuthHeaders()
            }
          );
...
const generarReporteConValidacion = async () => {
...
const calcularTotalPaginas = () => {
  ...
  if (!reporteForm.fechaDesde) {
    errores.push('La fecha desde es requerida');
  } else if (reporteForm.fechaDesde > hoy) {
    errores.push('La fecha desde no puede ser mayor a hoy');
  }
  
  if (!reporteForm.fechaHasta) {
    errores.push('La fecha hasta es requerida');
  } else if (reporteForm.fechaHasta > hoy) {
    errores.push('La fecha hasta no puede ser mayor a hoy');
  }
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content shadow-lg border-0">
              ...
              <div className="modal-body p-4">
                <form onSubmit={(e) => { e.preventDefault(); generarReporteConValidacion(); }}>
                  <div className="row">
                    <div className="col-md-6 mb-4">
                      <label className="form-label">
                        <i className="bi bi-calendar-date me-1" style={{ color: '#28a745' }}></i>
                        Fecha desde <span className="text-danger">*</span>
                      </label>
                      <input
                        type="date"
                        value={reporteForm.fechaDesde}
                        onChange={(e) => {
                          const nuevaFechaDesde = e.target.value;
                          setReporteForm(prev => ({
                            ...prev,
                            fechaDesde: nuevaFechaDesde,
                            fechaHasta: prev.fechaHasta && prev.fechaHasta < nuevaFechaDesde ? nuevaFechaDesde : prev.fechaHasta
                          }));
                        }}
                        className="form-control"
                    ...
                    <div className="col-md-6 mb-4">
                      <label className="form-label">
                        <i className="bi bi-calendar-check me-1" style={{ color: '#dc3545' }}></i>
                        Fecha hasta <span className="text-danger">*</span>
                      </label>
                      <input
                        type="date"
                        value={reporteForm.fechaHasta}
                        onChange={(e) => setReporteForm({...reporteForm, fechaHasta: e.target.value})}
                        className="form-control"
                        min={reporteForm.fechaDesde || undefined}
                        max={new Date().toISOString().split('T')[0]}
                        required
                      />
                    </div>
                  ...
                  <div className="row">
                    <div className="col-md-6 mb-4">
                      <label className="form-label">
                        <i className="bi bi-file-earmark-pdf me-1" style={{ color: '#6f42c1' }}></i>
                        Formato <span className="text-danger">*</span>
                      </label>
                      <select
                        value={reporteForm.formato}
                        onChange={(e) => setReporteForm({...reporteForm, formato: e.target.value})}
                        className="form-select"
                        required
                      >
                    ...
                    <div className="col-md-6 mb-4">
                      <label className="form-label">
                        <i className="bi bi-list-ul me-1" style={{ color: '#2196f3' }}></i>
                        Opciones adicionales
                      </label>
                      <div className="form-check mt-2">
                        <input
                          type="checkbox"
                          checked={reporteForm.incluirDetalles}
                          onChange={(e) => setReporteForm({...reporteForm, incluirDetalles: e.target.checked})}
                          className="form-check-input"
                          id="incluirDetalles"
                        />
                        <label className="form-check-label" htmlFor="incluirDetalles">
                          Incluir detalles de ingresos
                        </label>
                      </div>
                    </div>
                  </div>
                  
                  <div className="alert alert-info border-0" style={{ backgroundColor: '#cff4fc', borderLeft: '4px solid #0dcaf0' }}>
                    <div className="d-flex align-items-start">
                      <i className="bi bi-info-circle-fill me-2 mt-1" style={{ color: '#0dcaf0', fontSize: '1.1rem' }}></i>
                      <div>
                        <small>
                          El reporte incluirá todos los arqueos en el rango de fechas seleccionado.
                          {reporteForm.incluirDetalles && (
                            <span className="d-block mt-1" style={{ color: '#0a58ca' }}>
                              ✓ Se incluirán los detalles de ingresos de cada arqueo.
                            </span>
                          )}
                        </small>
                      </div>
              ...
              <div className="modal-footer bg-light border-0 p-3">
                <button 
                  ...
                  onClick={generarReporteConValidacion}
                  disabled={loading}
                >
                  {loading ? (
                    <>
